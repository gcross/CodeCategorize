...
void MinimalWeightOperatorSpace::formProductAndPostConstraints(
    ...
    for(int i = 0; i < product_X.size(); ++i) {
        rel(*this,product_non_trivial[i] == product_X[i] || product_Z[i]);
    };
    product_weight = expr(*this,sum(product_non_trivial));
...
void MinimalWeightOperatorSpace::multiplyOperators(
    ...
    for(int i = 0; i < Xin1.size(); ++i) {
        Xout[i] = expr(*this,Xin1[i]^Xin2[i]);
        Zout[i] = expr(*this,Zin1[i]^Zin2[i]);
    }
}
